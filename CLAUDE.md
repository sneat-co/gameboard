...
Monorepo for **GameBoard.live**. Three independent stacks, one repo:

- `backend/`  — Go (dalgo → Firestore)
...
**Cloudflare's own Git integration builds & deploys on every push to `main`.**
There is no GitHub Actions deploy step in the normal flow and **no `CLOUDFLARE_*`
secrets are needed** in this repo.
