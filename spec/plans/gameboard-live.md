...
### Rebuild slice plan (frontend, on the current Sneat scaffolding)

Each slice is gated by a **ported E2E** from `docs/legacy-mvp-frontend/` and built against the **already-live backend**:

- **Slice 0 — contract + service foundation** *(no UI)*: forward-port the event/state types + helpers from `contract.ts` into `libs/extensions/gameboard/contract`; extend `GameService` with `appendEvent` (`POST …/games/{id}/events`) and `getState` (`GET …/games/{id}/state`, anonymous). Unit-test the helpers (port `contract.spec.ts`).
- **Slice 1 — public scoreboard** *(do first; read-only, highest value)*: `ScoreboardPageComponent` at `/app/g/:gameID` renders the fold from `GET /state` (score, period, clock, fouls + bonus, timeouts, possession, on-court, minor-safe); `?display=big` big-screen mode. Gate: scoreboard E2E (seed state via the API, assert board == fold).
- **Slice 2 — operator console** (timekeeper + scorekeeper): `ConsolePageComponent` → `appendEvent` per action; re-read `GET /state` after each. Gate: port `full-game.spec.ts`.
- **Slice 3 — post-game recap** (box score): `RecapPageComponent` from the fold's box score, minor-safe. Gate: port `recap.spec.ts`.
- **Slice 4 — spectator / follow** (account-gated): follow team/player via `POST /follows`; anonymous can read/share, not follow. Gate: port `follow.spec.ts`.

Carry forward as-is: roster/consent stays the `DEMO_ROSTER` stub (no `sneat-team` substrate); live updates poll `GET /state` (true subscription deferred); ≥85% unit coverage on new logic.
